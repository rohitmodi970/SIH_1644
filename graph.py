...
def save_line_plot():
    ...
    plt.plot(df['month'], df['CO2'], marker='o', label='CO2 Emissions (kg)', color='royalblue', linewidth=2, markersize=8)
    plt.plot(df['month'], df['CH4']*12, marker='o', label='CH4 Emissions (kg)', color='orange', linewidth=2, markersize=8)
    plt.plot(df['month'], df['Total GHGs'], marker='o', label='Total GHGs (kg)', color='green', linewidth=2, markersize=8)
    plt.title('Monthly Greenhouse Gas Emissions', fontsize=16, fontweight='bold')
    plt.xlabel('Month', fontsize=12)
    plt.ylabel('Emissions (in tonne)', fontsize=12)
